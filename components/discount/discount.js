...
import '../FreeDelivery/FreeDelivery.scss'
...
const Discount = () => {
    ...
    const { Lang, setLang, arrayLang, discount, setDiscount, NewLandServi } = useBetween(state.useShareState);
    var today = new Date();
    ...
    const inputEvent = (event) => {
        ...
        if(name=="text" && (value > 100 || value < 0 ||  (!(Number(value) == value && value % 1 == 0) && !(Number(value) == value && value % 1 != 0))) ){
    ...
    return (
        <div className="deliveryCont" style={{ "direction": (Lang == 'Arabic' || Lang == 'pakistani') ? 'rtl' : 'ltr' }}>
            <h2>{arrayLang.desCount}</h2>
            <p> {arrayLang.textDisCountOffer}</p>
            <div className="deliveryList">
                <form className="form">
                    ...
                    <div style={{ display: "flex" }}>
                        <span className="text">{arrayLang.discountPrcentage} <span className="par">{arrayLang.percent}</span></span>
                    </div>

                    <input type="text" onChange={inputEvent} name="text" placeholder="0"
                        value={inputtext.text} />
                    <span className="par1"> %</span>
